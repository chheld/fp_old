...
public class TextViewJ extends TextView {
    ...
    private void justifyText() {
        ...
        if (!TextViewJUtils.isEmptyOrNull(text)) {
    ...
    private List<Line> justifyIterator(float viewWidth, String[] words,
        ...
        for (int i = 0; i < words.length; i++) {
            String word = words[i];
            if (TextViewJUtils.isEmptyOrNull(word)) {
                continue;
            }
            ...
            while (true) {
                // Post-word cases
                if (splitList != null
                        && !TextViewJUtils.isEmptyOrNull(splitList.get(post_word))) {
    ...
    private List<String> splitWord(String word, float wordSize, float viewWidth) {
        ...
        float charWidth = TextViewJUtils.getAverageCharacterWidth(mTextPaint, word);
        int numExceedChars = (int) TextViewJUtils.roundNumber((wordSize - viewWidth)
                / charWidth, 0);
    ...
    private float calculateSpacing(float viewWidth, Line line,
        ...
        if (wordsNumLimit > 1) {
            while (true) {
				 ...
				 */
                newSpacing = (float) TextViewJUtils.roundNumber(
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float f3 = f2 + i + j;
        updateParentLayoutParams((int) TextViewJUtils.roundNumber(f3, 0));
